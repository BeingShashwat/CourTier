...
public class CaseHtmlParser {
    ...
    private void parseHearingHistory(Case c, Document doc) {
        ...
        for (Element row : histTable.select("tr")) {
            ...
            HearingHistory hearing = new HearingHistory();
            hearing.setHearingDate(parseDate(cells.get(0).text().trim()));
            hearing.setPurpose(cells.get(1).text().trim());
            hearing.setJudgeName(cells.get(2).text().trim());
            hearing.setCourtCase(c);
            history.add(hearing);

            if (hearing.getHearingDate() != null
                    && (lastHearing == null || hearing.getHearingDate().isAfter(lastHearing))) {
                lastHearing = hearing.getHearingDate();
            }
        }
    ...
    private Case.CaseStatus deriveStatus(Document doc) {
        ...
        if (text.contains("disposed")) {
            return Case.CaseStatus.DISPOSED;
        }
        if (text.contains("transferred")) {
            return Case.CaseStatus.TRANSFERRED;
        }
